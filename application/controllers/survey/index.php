...
class index extends CAction
{
    ...
    public function action()
    {
        ...
        if( !empty(App()->request->getPost('age')) || App()->request->getPost('age') == "0" ){
            if((int)App()->request->getPost('age') < 1 || (int)App()->request->getPost('age') > 150){
                $_POST['move'] = $param['move'] = 'reload';
            } else if( !preg_match('/^[0-9]+$/', App()->request->getPost('age')) ){
                $_POST['move'] = $param['move'] = 'reload';
            }
        }
        if( isset($_POST['sno']) ){
            if(empty(App()->request->getPost('sno'))) {
                $_POST['move'] = $param['move'] = 'reload'; 
            } else {
                $tempData = Token::model($param['sid'])->find('attribute_3="'.App()->request->getPost('sno').'"');
                if($tempData && $tempData->token != trim($param['token'])){
                    $_POST['move'] = $param['move'] = 'reload'; 
                }
            }
            
        }
        // if( !empty(App()->request->getPost('sno')) || App()->request->getPost('sno') == "0" ){
        //     if( (int)App()->request->getPost('sno') < 1 ){
        //         $_POST['move'] = $param['move'] = 'reload';
        //     } else if( !preg_match('/^[0-9]+$/', App()->request->getPost('sno')) ){
        //         $_POST['move'] = $param['move'] = 'reload';
        //     }
        // }
        $surveyid    = $param['sid'];
        ...
        //     $aErrors    = array('Token mismatch');
        //     $asMessage  = array('The token you provided doesn\'t match the one in your session.');
        //     $aUrl       = array(
        //                     'url'=>$sReloadUrl,
        //                     'type'=>'restart-survey',
        //                     'description'=>"Click here to start the survey."
        //                     );
        ...
        // if($token){
        //     $tempUserData = Token::model($surveyid)->find('token="'.$token.'"');
        //     $thissurvey['userData'] = $tempUserData;
        // }
        ...
        // Owen
          // attribute_1 is age, attribute_2 is sex
        if(isset($token) && $move != 'movesubmit'){
            $tmpToken = $token;
            // $query = "SELECT * FROM {{tokens_716222}} WHERE token = '".$tmpToken."' AND (attribute_1 IS NULL || attribute_1 = '') AND (attribute_2 IS NULL || attribute_2 = '')";
            // $tempValue = Yii::app()->db->createCommand($query)->query()->read();
            $tempValue = Token::model($surveyid)->find('token="'.$tmpToken.'"');
            $redata['tokeninfo'] = $tempValue;
            $redata['tokenage'] = $tempValue->attribute_1;
            $redata['tokensex'] = $tempValue->attribute_2;
            $redata['tokensno'] = $tempValue->attribute_3;
            // if(empty($tempValue->attribute_3)) $tempValue->attribute_3 = 'N/A';
            // if($tempValue->usesleft == 999){ //檢查是否已做過
            //     $redata['firsttime'] = true;
            // }
            // if(App()->request->getPost('showagesex')){
            //     $tempValue->attribute_2 = $tempValue->attribute_1 = "";
            //     $tempValue->update();
            // }
            // if(empty($tempValue->attribute_2) && empty($tempValue->attribute_1)){
            //     $redata['no_age_sex'] = true;
            // } else {
            //     $redata['no_age_sex'] = false;
            // }
            if($tempValue->usesleft == 999){ //檢查是否已做過
                $redata['firsttime'] = true;
                $redata['need_age_sex_sno'] = true;
            } else {
                $redata['need_age_sex_sno'] = false;
            }
            $redata['age_sex_sno_msg'] = '';
            
            if($redata['need_age_sex_sno'] == true || App()->request->getPost('firsttime')){
                if( isset($_POST['sno']) && empty(App()->request->getPost('sno')) && $tempValue->attribute_3 != 'N/A' ){
                    $redata['age_sex_sno_msg'] = '病人編號為必填項';
                } else {
                    // 檢查 SNO 是否已存在
                    $tempData = Token::model($surveyid)->find('attribute_3="'.App()->request->getPost('sno').'"');
                    if(!empty(App()->request->getPost('sno')) && $tempData && $tempData->token != $tmpToken){
                        $redata['age_sex_sno_msg'] = '病人編號已存在，請輸入正確的編號';

                    } else {
                        if(!empty(App()->request->getPost('sno'))) $tempValue->attribute_3 = App()->request->getPost('sno');
                    
                        if( App()->request->getPost('age') || App()->request->getPost('age') == "0" || App()->request->getPost('sex') || App()->request->getPost('sno') || App()->request->getPost('sno') == "0" ){
                            if( !empty(App()->request->getPost('age')) || App()->request->getPost('age') == "0" ){
                                if((int)App()->request->getPost('age') < 1 || (int)App()->request->getPost('age') > 150){
                                    $redata['age_sex_sno_msg'] = '請填寫您的年齡';
                                } else if( !preg_match('/^[0-9]+$/', App()->request->getPost('age')) ){
                                    $redata['age_sex_sno_msg'] = '請輸入數字';

                                } else {
                                    App()->request->getPost('age') ? $tempValue->attribute_1 = (int)App()->request->getPost('age') : $tempValue->attribute_1 = 'N/A';
                                    App()->request->getPost('sex') ? $tempValue->attribute_2 = App()->request->getPost('sex') : $tempValue->attribute_2 = 'N/A';
                                    $redata['need_age_sex_sno'] = false;
                                }
                        ...
                        } else if(App()->request->getPost('firsttime')) {
                            // Yii::app()->setConfig('move', 'movenext');
                            $tempValue->attribute_1 = $tempValue->attribute_2 = 'N/A';
                            $redata['need_age_sex_sno'] = false;
                        }
                        $tempValue->update();
                    }
            ...
            if( App()->request->getParam('forcereload') == 'Y' ){
                $redata['need_age_sex_sno'] = false;
            }
        }
