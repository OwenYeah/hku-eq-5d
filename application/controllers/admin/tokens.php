...
class tokens extends Survey_Common_Action
{
  ...
  private function secToHR($seconds)
  {
    ...
    $seconds = $seconds % 60;
    return $hours > 0 ? "$hours" . "hrs" . " $minutes" . "mins " : ($minutes > 0 ? "$minutes" . "mins " . "$seconds" . "s" : "$seconds" . "s");
  }
  // Owen
  ...
  public function browse($iSurveyId, $limit = 50, $start = 0)
  {
    ...
    // Owen, need to optimize later
    $query = "SELECT s.id, token, submitdate, startdate, t.interviewtime
        FROM {{survey_$iSurveyId}} AS s
        LEFT JOIN {{survey_$iSurveyId}}_timings AS t ON s.id = t.id
        WHERE submitdate IS NOT NULL ORDER BY submitdate ASC";
    $row = Yii::app()->db->createCommand($query)->queryAll();
    $token_grp = [];
    foreach ($row as $key => $value) {
      if (!isset($token_grp[$value['token']])) $token_grp[$value['token']] = [];
      $token_grp[$value['token']][] = $value;
    }
    foreach ($token_grp as $key2 => $value2) {
      $string = '';
      foreach ($value2 as $key3 => $value3) {
        $string .= ($key3 + 1) . '. ' . date("Y-m-d H:i", strtotime($value3['startdate'])) . ' to ' . date("Y-m-d H:i", strtotime($value3['submitdate'])) . ', ' . $this->secToHR($value3['interviewtime']) . ' \n';
      }
      $query = "UPDATE {{tokens_$iSurveyId}} SET attribute_4 = '" . $string . "' WHERE token = '" . $key2 . "'";
      Yii::app()->db->createCommand($query)->query();
    }
    // $query = "UPDATE {{tokens_$iSurveyId}} SET attribute_4 = ".$string."WHERE token = '".$value['token']."'";
    // Owen
  ...
  private function _newtokentable($iSurveyId)
  {
    ...
    /* Restore a previously deleted tokens table */ elseif (returnGlobal('restoretable') == "Y" && Yii::app()->request->getPost('oldtable')) {
  ...
  public function genQrCode()
  {
    ...
    foreach ($aTokenIds as $key => $value) {
      $token = Token::model($iSid)->find('tid=' . $value);
      if ($token->sent != 'Y') {
        $count++;
        $oSurvey = Survey::model()->findByPk($iSid);
        $url = Yii::app()->createAbsoluteUrl('survey/index', array('sid' => $iSid, 'newtest' => "Y", 'lang' => $oSurvey->language, 'token' => $token->token), 'http');
        // $url = Yii::app()->getConfig('publicurl') . $this->getController()->createUrl("survey/index", array('sid'=>$iSid, 'newest'=>"Y", 'lang'=>$oSurvey->language, 'token'=>$token->token));
        QRcode::png($url, Yii::app()->getConfig('imagedir') . '/716222_tokens/' . $value . '_' . $token->token . '.png', 'L', 4, 2);
        $this->addTextToImage(Yii::app()->getConfig('imagedir') . '/716222_tokens/' . $value . '_' . $token->token . '.png', $token->token);
        $token->sent = 'Y';
        $token->update();
      }
    }
    ...
    if ($count > 0) {
      $zipFile = $this->createZip(Yii::app()->getConfig('imagedir') . '/716222_tokens/');
    }

    Yii::app()->getController()->renderPartial('/admin/token/massive_actions/_genqrcode_results', array('count' => $count, 'url' => Yii::app()->getConfig('imageurl') . '/716222_tokens/' . $zipFile));
  }
  ...
  private function createZip($path)
  {
    ...
    foreach ($files as $name => $file) {
      ...
      if (!$file->isDir() && $file->getExtension() == 'png') {
    ...
    // delete all files after finished
    foreach ($files as $name => $file) {
      // Skip directories (they would be added automatically)
      if (!$file->isDir() && $file->getExtension() == 'png') {
        unlink($file);
  ...
  private function addTextToImage($path, $text = 'demo')
  {
    $width = 164;
    $height = 164;
    // Set the content-type
    $main = imagecreatetruecolor($width, $height + 30);
    ...
    // Font path
    $font = Yii::app()->getConfig('fontdir') . '/font-src/' . Yii::app()->getConfig("alternatechartfontfile")['hi'];
    // Add the text
    imagettftext($im, 20, 0, 5, 25, $black, $font, $text);
    $this->imagecopymerge_alpha($main, $qr, 0, 0, 0, 0, $width, $height, 100);
    $this->imagecopymerge_alpha($main, $im, 0, $width, 0, 0, $width, 30, 100);
    imagepng($main, $path);
    imagedestroy($main);
  }
  ...
  // Owen
  public function genReportOfAll()
  {
    Yii::app()->loadHelper("export");
    $iSurveyID = Yii::app()->getRequest()->getPost('sid');
    $queryString = "SELECT `attribute_3`,`attribute_2`, `attribute_1`, `lime_survey_716222`.`submitdate` AS submitdate, `716222X1X1`, `716222X1X2`, `716222X1X3`, `716222X1X4`, `716222X1X5`, `716222X2X6` FROM `lime_survey_716222` 
      LEFT JOIN `lime_tokens_716222` ON lime_tokens_716222.token = lime_survey_716222.token
      WHERE `attribute_4` is not null 
      AND `attribute_3` is not null 
      AND (`716222X1X1` is not null OR `716222X1X2` is not null OR `716222X1X3` is not null OR `716222X1X4` is not null OR `716222X1X5` is not null OR `716222X2X6` is not null)
      ORDER BY `attribute_3`, submitdate";
    $rows = Yii::app()->db->createCommand($queryString)->queryAll();
    ...
    $tokenoutput .= 'subject no.,Sex,Age,date,"time (T0, T1…)",mobility,self-care,usual activities,pain/discomfort,anxiety/depression,Utility score, VAS';
    $tokenoutput .= "\n";
    echo $tokenoutput;
    $tokenoutput = '';
    $current_subject_no = '';
    ...
    foreach ($rows as $value) {
      $tokenoutput .= '"' . trim($value['attribute_3']) . '",';
      $tokenoutput .= '"' . trim($value['attribute_2']) . '",';
      $tokenoutput .= '"' . trim($value['attribute_1']) . '",';
      $tokenoutput .= '"' . date("Y-m-d", strtotime($value['submitdate'])) . '",';
      if ($current_subject_no != $value['attribute_3']) {
        $current_subject_no = $value['attribute_3'];
        $count = 0;
      } else {
        $count++;
      }
      $tokenoutput .= '"' . trim($count) . '",';
      $tokenoutput .= '"' . trim(str_replace('A', '', $value['716222X1X1'])) . '",';
      $tokenoutput .= '"' . trim(str_replace('A', '', $value['716222X1X2'])) . '",';
      $tokenoutput .= '"' . trim(str_replace('A', '', $value['716222X1X3'])) . '",';
      $tokenoutput .= '"' . trim(str_replace('A', '', $value['716222X1X4'])) . '",';
      $tokenoutput .= '"' . trim(str_replace('A', '', $value['716222X1X5'])) . '",';
      $tokenoutput .= '"' . trim(str_replace('A', '', $value['716222X2X6'])) . '",';
      // Start - Calculate the utility score
      $sum = 0;
      $isEmtry = 0;
      $healthstate = 'N/A';
      foreach ($summary as $key2 => $value2) {
        ...
        if ($value[$value2]) {
          $query = "SELECT assessment_value FROM {{answers}} WHERE qid = " . $qqid . " AND code = '" . $value[$value2] . "'";
          $value[$value2 . '-a'] = Yii::app()->db->createCommand($query)->query()->read()['assessment_value'] / 1000.0;
          $sum += $value[$value2 . '-a'];
        } else {
          $isEmtry++;
        }
      ...
      $tokenoutput .= '"' . trim($healthstate) . '",';
      // End - Calculate the utility score
      // $tokenoutput .= '"' . trim(str_replace('A', '', $value['716222X1X2'])) . '"';
      $tokenoutput .= "\n";
      echo $tokenoutput;
      $tokenoutput = '';
