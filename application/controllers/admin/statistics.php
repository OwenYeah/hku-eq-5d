...
class statistics extends Survey_Common_Action
{
     ...
     * @param array $aData Data to be passed on. Optional.
     */
    public function statisticsByToken($surveyid, $token = ""){

	    $aData = [];
        $aData['surveyid'] = $surveyid;
        $aData['raw_data'] = [];
        if(Yii::app()->request->getPost('token', 0)){
            $token = Yii::app()->request->getPost('token', 0);
            $aData['token'] = $token;
        }
        ...
        if(!empty($token)){
            ...
            $tokenData = Token::model($surveyid)->find('token="'.$token.'"');
            // 20200601: Change to search by sno
            ...
            // End
            $query = "SELECT token, submitdate, 716222X1X1, 716222X1X2, 716222X1X3, 716222X1X4, 716222X1X5, 716222X2X6 FROM {{survey_$surveyid}} WHERE token = '$token' AND submitdate IS NOT NULL ORDER BY submitdate DESC LIMIT 6";
        ...
        $aData['downloadpdf'] = Yii::app()->createUrl("/assets/pdfs/".$this->downloadStatisticsPDF($surveyid, $token));

        echo $this->_renderWrappedTemplate('export', 'hku_statistics', $aData);

        
        // $iSurveyID = (int)$surveyid;
        // $sBaseLanguage = Survey::model()->findByPk($iSurveyID)->language;
        // $aSurveyInfo = getSurveyInfo($iSurveyID, $sBaseLanguage);
        // echo $this->_renderHtml($sMessage, $aSurveyInfo, $iSurveyID);

        // echo $this->_renderWrappedTemplate('export', 'hku_statistics', $aData);

        // Explode the table field, and get the attribute of question and answer
     ...
     * @param array $aData Data to be passed on. Optional.
     */
    public function statisticsBySno($surveyid, $sno = ""){

        $aData = [];
        $aData['surveyid'] = $surveyid;
        $aData['raw_data'] = [];
        // 20200601: Change to search by sno
        if(Yii::app()->request->getPost('sno', 0)){
            $sno = Yii::app()->request->getPost('sno', 0);
            // $query = "SELECT distinct token FROM {{survey_$surveyid}} WHERE attribute_3 = '$sno'";
            // $row = Yii::app()->db->createCommand($query)->queryAll();
            $tokenData = Token::model($surveyid)->find('attribute_3="'.$sno.'"');
            $token = $tokenData->token;
        } else{
            print_r('Subject not exists');
            exit();
        }
        ...
        if(!empty($token)){
            ...
            // End
            $query = "SELECT token, submitdate, 716222X1X1, 716222X1X2, 716222X1X3, 716222X1X4, 716222X1X5, 716222X2X6 FROM {{survey_$surveyid}} WHERE token = '$token' AND submitdate IS NOT NULL ORDER BY submitdate DESC LIMIT 8";
            $row = Yii::app()->db->createCommand($query)->queryAll();
            if(sizeof($row)){
                $summary=['716222X1X1', '716222X1X2', '716222X1X3', '716222X1X4', '716222X1X5'];
                $newRow = [];
    ...
    private function downloadStatisticsPDF($surveyid, $token = ""){
        $wkhtmltopdf  = '/usr/local/bin/wkhtmltopdf';
        // $wkhtmltopdf .= ' --no-outline --header-html "'.base_url('print/invoice/header/invoice_id/'.$thisInsertId.'/language/'.$thisPOST['invoice_language']).'"';
        $wkhtmltopdf .= ' --zoom 0.8 --no-outline --margin-top 10 --header-spacing 0 --javascript-delay 2000 --enable-javascript --no-stop-slow-scripts "'.Yii::app()->createAbsoluteUrl("/hku_statistics/".$surveyid.'/'.$token).'"';
        $filename = $surveyid.'_'.$token.'_'.time().'.pdf';
        $wkhtmltopdf .= ' assets/pdfs/'.$filename;
        // echo $wkhtmltopdf;
