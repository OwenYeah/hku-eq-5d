...
<div class='side-body <?php echo getSideBodyClass(false); ?>'>
    ...
    <?php $pageSizeTokenView=Yii::app()->user->getState('pageSizeTokenView',Yii::app()->params['defaultPageSize']);?>
        ...
        <!-- Grid -->
        <div class="row">
            <div class="content-right scrolling-wrapper"  onchange="test()" >
                <?php
...
<script type="text/javascript">
    ...
    //     console.log("tset");
    // }
    $("body").on('DOMSubtreeModified', "#token-grid", function() {
        // alert('changed');
        if(!$("#token-grid").hasClass("loaded")){
           $("#token-grid > table > tbody > tr > td:last-child").each(function(index){
              var text = $(this).text();
                $(this).html("<pre style='border: 0; background: transparent; padding: 0; margin: 0'>"+text+"</pre>");
                $("#token-grid").addClass("loaded");
            }) 
        }
    });
