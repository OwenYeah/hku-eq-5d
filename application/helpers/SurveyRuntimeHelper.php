...
class SurveyRuntimeHelper
{
    ...
    public function run($surveyid, $args)
    {
        ...
        if($need_age_sex_sno) {
            $this->aSurveyinfo['showwelcome'] = 'Y';
            $_SESSION[$this->LEMsessid]['step'] = 0;
        } else  {
            if($_SESSION[$this->LEMsessid]['step'] < 1) {
        ...
        if (!$this->previewgrp && !$this->previewquestion) {
            ...
            //owen
            if(!$this->aSurveyInfo['firsttime'] && $_SESSION[$this->LEMsessid]['step'] <= 1) {
                $this->aSurveyInfo['agesexprev'] = false;
            }
            //owen
        ...
        // owen
        if($_SESSION[$this->LEMsessid]['step'] >= 1 && !$need_age_sex_sno){
            $url = Yii::app()->getController()->createUrl("/survey/index/sid/{$surveyid}", array('token'=>$clienttoken, 'lang'=>App()->language, 'newtest'=>'Y', 'forcereload'=>'Y'));
            if(empty($_SESSION[$this->LEMsessid]['fieldmap'])) header("Refresh:0; url=".$url);
            $this->aSurveyInfo['clienttoken'] = $clienttoken;
        }
        // owen
...
/* Can use aQuestionAttributes too */
    ...
    private function setSurveySettings($surveyid, $args)
    {
        ...
        // owen
        $this->aSurveyInfo['tokenage'] = $tokenage;
        $this->aSurveyInfo['tokensex'] = $tokensex;
        $this->aSurveyInfo['tokensno'] = $tokensno;
        $this->aSurveyInfo['firsttime'] = $firsttime;
        $this->aSurveyInfo['age_sex_sno_msg'] = $age_sex_sno_msg;
        $this->aSurveyInfo['agesexprev'] = true;
        $this->aSurveyInfo['need_age_sex_sno'] = $need_age_sex_sno;
        // owen
